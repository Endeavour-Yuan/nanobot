...
Speaks MCP over stdio to the `cua-driver` binary. The Python `mcp` SDK is
async, so we run a dedicated asyncio event loop on a background thread and
marshal sync calls through it.
...
_CUA_DRIVER_CMD = os.environ.get("CUA_DRIVER_CMD", "cua-driver")
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or CUA_DRIVER_CMD resolves."""
...
def cua_driver_install_hint() -> str:
    return (
        ...
        "  nanobot computer-use install\n"
        "Or run the upstream installer directly:\n"
        '  /bin/bash -c "$(curl -fsSL '
        'https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh)"\n'
        "Or check the nanobot docs for setup instructions."
    )
